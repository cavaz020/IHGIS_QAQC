...
def validate_total(agg_col_data, total_col_data, data_df):
    # this function adds the aggregating columns together and checks it with the total column
    agg_col_data['summed'] = agg_col_data.iloc[:, :].sum(axis=1)
    total_col_data['total'] = total_col_data.iloc[:, :]
    # agg_col_data['true_val'] = agg_col_data.loc[agg_col['summed'] == total_col_data['total'], 'True']
    # agg_col_data['false_val'] = agg_col_data.loc[agg_col['summed'] != total_col_data['total'], 'False']
    # comparison_column = agg_col_data['true_val'] + agg_col_data['false_val']
...
# ---------------------------------------------------------------------------------
agg_example_df = pd.DataFrame()
# total_col_data example
tot_example_df = pd.DataFrame([(1)], columns=['A'])


sanity_check_col_lengths = check_dfs(agg_example_df, tot_example_df)
# print(sanity_check_col_lengths)
...
validated_data = validate_total(
    agg_column_data, total_column_data, data_only_df)
